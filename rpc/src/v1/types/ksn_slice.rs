...
pub struct KsnSlice {
	...
	pub metadata: Option<KsnSliceMetadata>,
	/// Trie nodes of the slice
	pub trie_nodes: Option<KsnSliceNodes>,
	/// If the slice is a state one (as opposed of storage),
	/// we return the leaves' storage root and EVM code of
	/// the smart contracts
	pub leaves: Option<BTreeMap<H256, KsnSmartContract>>
}

/// KSN slice metadata representation.
/// Time in milliseconds and number of trie nodes
#[derive(Debug, Serialize)]
#[serde(rename_all="kebab-case")]
pub struct KsnSliceMetadata {
...
/// KSN slice trie nodes representation.
/// Stem (including the trie root), head of the slice,
/// and the slice nodes
#[derive(Debug, Serialize)]
#[serde(rename_all="kebab-case")]
pub struct KsnSliceNodes {
...
mod tests {
	...
	use std::collections::BTreeMap;
	use std::str::FromStr;
	use super::{KsnSlice, KsnSliceMetadata, KsnSliceNodes, KsnSmartContract};
	use v1::types::{Bytes, H256};

	#[test]
	fn test_serialize_ksn_slice() {
		let ksn_slice = KsnSlice{
			slice_id: "1372-10-8e9e2d4514bee72525133350201ecfc3da3815a4bb6b03ad5ff3bdd555363188".into(),
			metadata: Some(KsnSliceMetadata{
				time_ms: map![
